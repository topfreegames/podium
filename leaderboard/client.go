...
func NewClient(host string, port int, password string, db int) (service.Leaderboard, error) {
...
func NewClusterClient(host string, port int, password string, db int, connectionTimeout int) (service.Leaderboard, error) {
	redisInstance := database.NewRedisDatabase(database.RedisOptions{
		ClusterEnabled: true,
		Host:           host,
		Port:           port,
