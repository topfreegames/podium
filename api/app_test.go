...
var _ = Describe("App", func() {
	...
	Describe("App creation", func() {
		It("should create new app", func() {
			app, err := api.GetApp("127.0.0.1", 9999, 10000, "../config/test.yaml", true, false, logger)
			Expect(err).NotTo(HaveOccurred())
			Expect(app).NotTo(BeNil())
			Expect(app.Host).To(Equal("127.0.0.1"))
			Expect(app.HTTPPort).To(Equal(9999))
	...
	Describe("App Load Configuration", func() {
		It("Should load configuration from file", func() {
			app, err := api.GetApp("127.0.0.1", 9999, 10000, "../config/test.yaml", false, false, logger)
			Expect(err).NotTo(HaveOccurred())
			Expect(app.Config).NotTo(BeNil())
		...
		It("Should fail if configuration file does not exist", func() {
			app, err := api.GetApp("127.0.0.1", 9999, 10000, "../config/invalid.yaml", false, false, logger)
			Expect(app).To(BeNil())
	...
	Describe("App Connect To Redis", func() {
		It("Should faild if invalid redis connection", func() {
			app, err := api.GetApp("127.0.0.1", 9999, 10000, "../config/invalid-redis.yaml", false, false, logger)
			Expect(app).To(BeNil())
			Expect(err).To(HaveOccurred())
